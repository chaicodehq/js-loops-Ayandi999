...
export function upiRetry(outcomes) {
  ...
  let i=0
  let timer=1;
  let result={attempts: 0, success: false, totalWaitTime: 0}
  do{
    //success
    ...
    //failure
      ...
      result.attempts++; //1->2->3->4
      if(result.attempts>=5) return result;
      result.totalWaitTime+=timer; //1->3->7->15
      timer+=timer; //2->4->8->16
    }
    i++;
  }while(i<len);
