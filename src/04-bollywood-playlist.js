...
export function buildPlaylist(songs, maxDuration) {
  ...
  let totalDuration=0,songCount=0
  for(var i of songs){
    
    if(i>0 && totalDuration+i <= maxDuration){
      totalDuration+=i;
      songCount++;
    }else if(totalDuration+i>maxDuration){
      break;
    }else{
      continue
    }
  }return {count:songCount,totalDuration:totalDuration}
}
